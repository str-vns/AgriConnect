...
const DMonthlyProductsAdmin = () => {
    ...
    const getSpecificProducts = async () => {
        ...
        try {
            const { data } = await axios.get(`http://localhost:4000/api/v1/stocky`, config);
    ...
    return (
        <div>
            <div style={{ backgroundColor: 'white', padding: '30px', borderRadius: '8px', textAlign: 'center' }} ref={pdfRef}>
                {specificItem.length > 0 && (
                    <p className='text-center ml-20 font-bold'>{getMonthName(specificItem[0].month)}</p>
                )}
            ...
            <button  class="btn btn-primary" onClick={downloadPDF} style={{ marginTop: '20px' }}>
               Download PDF
            </button>
        </div>
    );
...
export default DMonthlyProductsAdmin;
