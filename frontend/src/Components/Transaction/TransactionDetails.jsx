...
const OrderDetails = () => {
 ...
 <p className={`ml-2 ${order.items && order.items.some(item => item.orderStatus === 'Processing') ? "text-blue-600" : "text-green-600"}`}>
                    <b>{order.items && order.items.some(item => item.orderStatus === 'Processing') ? "Processing" : "Delivered"}</b>
                </p>
</div>
