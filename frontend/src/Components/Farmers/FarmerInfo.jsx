...
// console.log(friend?.user?.avatar?.url);
 ...
 useEffect(() => {
  const getFarmerProducts = async (frdyyId) => {
    try {
      if (frdyyId) {
        const { data } = await axios.get(`http://localhost:4000/api/v1/farmerprod/${frdyyId}`);
         ...
         <div class=" py-8   flex flex-wrap items-center  justify-center  ">
            <div class="container rounded-lg lg:w-5/6 xl:w-2/7 sm:w-full md:w-2/3    bg-white  shadow-lg    transform   duration-200 easy-in-out">
               <div class=" h-48  overflow-hidden" >
                  <img class="w-full" src={farmerloc?.farmersloc?.images[0].url} alt="" />
               </div>
               <div class="flex justify-right px-5 items-end -mt-12">
                  <img class="h-32 w-32 bg-white p-2 ring-2 ring-black rounded-full" src={friend?.user?.avatar?.url} alt="" />
                  <div class="mt-14 ml-14">
                     <h2 class="text-gray-800 text-3xl font-bold">{friend?.user?.name}</h2>
                     <h1 class="text-gray-400 ">{friend?.user?.email}</h1>
                  </div>
               </div>
               <div className="text-center my-10">
                  <h1 className="text-3xl font-bold mb-8 text-black">Products Available</h1>
